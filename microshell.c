#include "microshell.h"

int ft_strlen(char *str)
{
	int i = 0;

	while (str[i])
		i++;
	return (i);
}

void    print_error(char *str, char *str1, int n)
{
	write(2, str, ft_strlen(str));
	if (str1)
		write(2, str1, ft_strlen(str1));
	write(2, "\n", 1);									////////////////////////
	if (n)
		exit(1);
}

t_tokens    *init_tokens()
{
	t_tokens *tmp;
	if (!(tmp = malloc(sizeof(t_tokens))))
		print_error("error: fatal", NULL, 1);
	if (!(tmp->cmds = malloc(sizeof(char *) * 1024)))
		print_error("error: fatal", NULL, 1);
	tmp->std_in = 0;								// ***
	tmp->std_out = 1;								// ***
	tmp->next = NULL;
	return (tmp);
}

void    ft_pipe(t_tokens *tokens)
{
	int pip[2];

	pipe(pip);
	tokens->next = init_tokens();
	tokens->next->std_in = pip[0];
	tokens->std_out = pip[1];
}

int add_cmd(t_tokens *tokens, char **argv, int i)
{
	int j = 0;

	while (argv[i] && strcmp(argv[i], ";") && strcmp(argv[i], "|"))
	{
		tokens->cmds[j] = argv[i];
		i++;
		j++;
	}
	i++;
	tokens->cmds[j] = NULL;
	return (i);
}

void    exec_cmd(t_tokens *tokens, int *start, int i)
{
	int pid;

	if (strcmp(tokens->cmds[0], "cd") == 0)						// cd
	{
		if (tokens->cmds[2] || !tokens->cmds[1])
			print_error("error: cd: bad arguments", NULL, 0);
		else if (chdir(tokens->cmds[1]) == -1)
			print_error("error: cd: cannot change ", tokens->cmds[1], 0);
		*start = i;
		return ;
	}

	pid = fork();												// pid error
	if (pid < 0)
		print_error("error: fatal", NULL, 1);

	if (pid == 0)												// hijo
	{
		if (tokens->std_out != 1)
		{
			close(tokens->next->std_in);
			if (dup2(tokens->std_out, 1) == -1)
				print_error("error: fatal", NULL, 1);
		}
		if (tokens->std_in != 0)
			if (dup2(tokens->std_in, 0) == -1)
				print_error("error: fatal", NULL, 1);
		execve(tokens->cmds[0], tokens->cmds, NULL);
		print_error("error: cannot execute ", tokens->cmds[0], 1);
	}

	if (tokens->std_out != 1)									// padre
		close(tokens->std_out);
	if (tokens->std_in != 0)
		close(tokens->std_in);
	if (tokens->std_out == 1)
	{
		while (*start < i)
		{
			waitpid(0, NULL, 0);
			(*start)++;
		}
	}
}

void    ft_free(t_tokens *tokens)
{
	if (tokens)
	{
		if (tokens->cmds)
			free(tokens->cmds);
		free(tokens);
		tokens = NULL;
	}
}

int     main(int argc, char **argv)
{
	t_tokens* tokens = NULL, *before = NULL;
	int start = 1, i = 1;

	while (i < argc)
	{
		if (tokens == NULL || start == i)					// init
			tokens = init_tokens();

		if (strcmp(argv[i], ";") == 0)						// ;
		{
			i++;
			continue;
		}

		i = add_cmd(tokens, argv, i);						// add

		if (argv[i - 1] && strcmp(argv[i - 1], "|") == 0)	// *** pipe  ***
			ft_pipe(tokens);

		exec_cmd(tokens, &start, i);						// exec

		before = tokens;									// *** clean  ***
		if (tokens->std_out != 1)							//////// ***** ///////
			tokens = tokens->next;
		ft_free(before);
	}
	return (0);
}
